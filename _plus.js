...
_.mixin({
	 ...
	 * the rest of elements are appended at the end; the comparison are donw with the equals operator ==. 
	 */
	orderBy: function(arr, comparator_or_orderd_items){
		if(_.isNotEmptyArray(arr)){
			if(_.isFunction(comparator_or_orderd_items)){
				var ret = new Array();
				_.each(arr, function(elem){
					var pos = -1;
					for(var i=0; i<ret.length && pos<0; i++){
						if(comparator_or_orderd_items.call(null,elem,ret[i])<0)
							pos = i;
					}
					if(pos>=0)
			...
			else if(_.isNotEmptyArray(comparator_or_orderd_items)){
				var elems = _.clone(arr);
				var ret = new Array();
				_.each(comparator_or_orderd_items, function(ordered_item){
		...
		if(_.isObject(obj)){
			...
			if(merge_objects && (_.is(new_value)) && (_.is(obj[key]))){
				if(_.isObject(new_value) && _.isObject(obj[key]))
					_.extendObj(obj[key],new_value,merge_objects,add_objects);
				else
...
_.mixin({
    ...
    dateStr: function(date, include_hour_or_hour_fields, usa_format){
		...
		var ret = (!!usa_format) ? (date.getFullYear()+"-"+_.dateFieldStr(date.getMonth()+1)+"-"+_.dateFieldStr(date.getDate())) :
				(_.dateFieldStr(date.getDate())+"/"+_.dateFieldStr(date.getMonth()+1)+"/"+date.getFullYear());
		if(!!include_hour_or_hour_fields){
			var fields = _.isNumber(include_hour_or_hour_fields) ? include_hour_or_hour_fields : 2;
			if(fields>0)
				ret+=" "+_.dateFieldStr(date.getHours()) + ( (fields>1) ? ":"+_.dateFieldStr(date.getMinutes()) : "" ) + 
						( (fields>2) ? ":"+_.dateFieldStr(date.getSeconds()) : "" ) + ( (fields>3) ? ":"+_.dateFieldStr(date.getMilliseconds()) : "" );
		}
		return ret;
	},
